...
impl PinLayout {
    pub fn from_config(layout: &LayoutConfig) -> PinLayout {
        let result = PinLayout {
            ...
            toggle_valves: layout
                .get_valves()
                .iter()
                .map(|valve_conf| ToggleValve::from_config(valve_conf))
                .collect(),
        };
    ...
    pub fn find_pin(&self, valve_pin_num: u64) -> Result<&ToggleValve, ()> {
        match self
            .get_valve_pins()
            .iter()
            .find(|ref valve_pin| valve_pin_num == valve_pin.get_valve_pin().get_pin())
        {
            None => Err(()),
            Some(pin) => Ok(pin),
        }
    ...
    pub fn get_button_streams(&self) -> impl Future<Item = (), Error = ()> {
        ...
        let mut valves: Vec<ToggleValve> = Vec::new();
        for pin in valve_pins {
            valves.push(pin.clone())
        }
        return lazy(move || {
